...
Hi, my name is MFGAVIN and the reason for this guide was that Shadowplay worked horribly for me, the audio as well as the video quality were terrible. So I tried other alternatives, mainly Medal.tv but once again it was a horrible experience. After that, I was stumped since there wasn't any software I considered to be good enough, and didn't really know what I should do. However; after some time, I decided to try and make my own through OBS, and well now you know I was able to.

# Disclaimer <a name = "disclaimer"><a/>

First, I'd like to state that this guide is WIP unless I state otherwise. I'd like to hear all criticism as this is my first time doing something like this. Also, if you have anything to tell me, use the _Issues_ on GitHub
...
If others want to use this guide in something, ex: Youtube, then I ask you to provide credits to me as well as all the sources I state in [Credits & Cited Sources](#citations).
...
This part only really matters if you have a bunch of scenes and stuff already set up and don't want to screw it up.

So in the main window of OBS if you look at the top you will see **Profile** and **Scene Collection**. The purpose of these is to make it able to have completely different settings and scenes apart from your usual stuff.
...
This part is also vital since it'll optimize the resources when switching between Desktop and Game capture.

**Note:** Whenever you set everything up you may notice sometimes clips go into folders they shouldn't be. This can be caused by trying to **Save Replay** but Alt+Tab'ing out which causes the clip to be prefixed with another scene names
...
What the script does is it makes it able to make folders and or files based on the scene names. Sadly, the person who made the script was last active on July 16, 2021. And so they're two bugs that I found that I want you guys to be aware of.
...
2.  The second bug occurs when selecting the settings for the script so I'll go over it more during the setup.
...
5.  Add the **OBSPlay script** from wherever you're keeping it
...
1. Clips will be prefixed with the scene name and then go into a singular folder
...
Look at **Audio Mixer** click the **Gear Wheel** and select **Advanced Audio Properties**
...
**Set the recording path to the hard drive where you want your clips to be stored**, for me it's F:/ but for others, it may be C:/ , D:/.
...
**For Audio Tracks**, you can either have both 1 and 2 turned on or just 1 it just depends on your use case since having 2 tracks, one for mic and one for desktop audio allows you to personally customize them later on in editing but it's up to you.

**Encoder Options** are going to be slightly varied so I won't discuss too much about it. If you have a RTX card go with **NVIDIA NVENC H.264 (new)** since it'd be the preferred use case for me but for you, it may be different, so search up if it could be.

**Rescale Output**, leave unchecked, but for some people you may have to use it. Search it up if you may have to because of something like monitor resolutions
...
**Keyframe Interval** I've seen a couple of numbers thrown around so either set it to **0, 2, or 5**. Though as you can see I set it to 2.

**Preset** can either be either **Max-Quality** or **Quality**. You can try the others but I've heard that \*\*Low-Latency options don't do much but once again test it out and see if it's something you want to go.
...
- **Bicubic** is a good balance between resource use and image quality
...
5. Click **Analyze**
6. This will bring you to a website that displays **Critical Issues, Warnings, and Info** and will help you debug by stating the problems arising in your case as well as stating solutions to them. It has also helped me fix some bugs that I found throughout my journey.

> But MFGAVIN, this shit still ain't working!

So if all of this still hasn't worked then I'd like to direct you toward the [OBS Discord Server](https://discord.gg/obsproject) which has support desks. I left this as the last option since depending on how good this guide does I don't want to overflow the discord with the same shit.

# Making OBS Open on Startup <a name = "obsAutoStart"><a/>

**Thanks to Koala for letting me use this guide they made for here.**

With this guide, you will be able to run OBS as an administrator on login without an elevation prompt by using the task scheduling feature of Windows.

You should find the **Task Scheduler** by searching for "task scheduler" in **Windows Search**. It should find your localized name for the task scheduler.  
...
It's important to **"Turn only when user is logged on"**. If you don't do this, OBS will not appear on the desktop but run only in the background.  
...
You can add arguments as well such as, **--startreplaybuffer --minimize-to-tray**
...
You can even put an icon on the desktop that, upon clicking, will start this task manually, so you start OBS Studio as administrator without an elevation prompt. Create a new shortcut with this target:
...
After looking for a bit and procrastinating I found two scripts that do the same thing but play the sound at different points
